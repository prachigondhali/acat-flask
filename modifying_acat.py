...
app = Flask(__name__)
#read csv
df = pd.read_csv('completeData.csv', usecols = headers)
...
headers = ['business_classification', 'business_impact', 'business_relevance',
       ...
       'IsDatabaseSupported', 'pivot.disposition_1']

dependent_variable = headers[-1]

model_directory = 'model'
model_file_name = '%s/model.pkl' % model_directory
model_columns_file_name = '%s/model_columns.pkl' % model_directory

model_columns = headers
classifier = None
...
def train():
	...
	for f in headers:
    df[f] = d[f].astype('category')
    categories[f] = df[f].cat.categories
